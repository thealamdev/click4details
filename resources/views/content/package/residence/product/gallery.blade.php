...
@section('content')
    <div class="container">
        ...
        @if (is_object($residence->gallery) && $residence->gallery->count() > 0)
            <div class="row row-cols-1 row-cols-lg-3 row-cols-xl-5 g-3 mb-3">
                @foreach ($residence->gallery as $each)
                    <div class="col">
                        <div class="card">
                            ...
                            <div class="card-body d-flex justify-content-between">
                                ...
                                <div>
                                    <form action="{{ route('admin.residence.product.gallery.delete', ['residence' => $residence->id, 'gallery' => $each->id]) }}" method="POST">
                                        @csrf
                                        @method('delete')
        ...
        <div class="card rounded border border-gray-300">
            <div class="card-body">
                <form action="{{ route('admin.residence.product.gallery.store', ['residence' => $residence->id]) }}" method="POST" accept-charset="utf-8" autocomplete="on" class="need-validation" novalidate enctype="multipart/form-data">
                    @csrf
