...
@section('title', 'Rent Car Gallery')
@section('content')
    <div class="container">
        <div class="d-flex align-items-center justify-content-between">
            <div>
                <h4 class="mb-0">Rent Car Gallery</h4>
                <p>Prior to create a new role, make sure asterisk (*) signs are filled</p>
            </div>
        </div>

        @if (is_object($rentCar->gallery) && $rentCar->gallery->count() > 0)
            <div class="row row-cols-1 row-cols-lg-3 row-cols-xl-5 g-3 mb-3">
                @foreach ($rentCar->gallery as $each)
                    <div class="col">
                        <div class="card">
                            <div class="card-body pb-0">

                                <img src="{{ $each->preview() ?? 'https://via.placeholder.com/600x400/c9d2e3/212837' }}" alt="" class="card-img-top rounded" />
                            </div>
                            <div class="card-body d-flex justify-content-between">
                                ...
                                <div>
                                    <form action="{{ route('admin.rental.service.car.gallery.delete', ['rentCar' => $rentCar?->id, 'gallery' => $each?->id]) }}" method="POST">
                                        @csrf
                                        @method('delete')
        ...
        <div class="card rounded border border-gray-300">
            <div class="card-body">
                <form action="{{ route('admin.rental.service.car.gallery.store', ['rentCar' => $rentCar->id]) }}" method="POST" accept-charset="utf-8" autocomplete="on" class="need-validation" novalidate enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row gx-3">
                        <div class="col-12 col-md-12">
                            <div class="form-group mb-3">
                                ...
                                <input type="file" name="image[]" class="form-control @error('image') {{ 'is-invalid' }} @enderror" multiple accept=".png, .jpg, .jpeg" required />
                                @error('image')
