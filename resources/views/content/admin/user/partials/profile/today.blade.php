        ...
        <div class="col-lg-12">
            ...
            <div class="card-body">
                <div class="row reloadBody">
                    ...
                    @if (is_object($schedule) && count($schedule) > 0)
                        @foreach ($schedule as $each)
                            <div class="col-lg-1">
                                @if ($each?->customer?->name !== null)
                                    <label
                                        class="fs-6 fw-500">{{ $each?->customer?->name }}</label>
                                    <span
                                        class="fs-6 fw-400">{{ $each?->customer?->mobile }}</span>
                                    <a id="bootModalShow"
                            ...
                            <div class="col-lg-5">
                                @if ($each?->message !== null)
                                    <label
                                        class="fs-6 fw-500">{{ Str::limit(optional($each)->message, 100, '...') }}
                                    </label>
                                @endif
                                <div class="row">
                                    <div class="col-lg-12">
                                                ...
                                                @foreach ($each->customerFollowupMessageFeedback as $feedback)
                                                    <li class="fs-6 fw-400 px-2 pt-2">
                                                        ...
                                                        <div class="feedback-message">
                                                            ...
                                                            {{ $feedback->budget }}
                                                            <span
                                                                class="ps-5 fs-6 fw-400 text-primary">
                                                                <i
                            ...
                            <div class="col-lg-1">
                                <div class="d-flex">
                                    ...
                                    <form
                                        ...
                                        @else
                                            <button type="submit"
                                                {{ is_null($each->call_time) ? 'disabled' : '' }}
                            ...
                            <div class="col-lg-1">
                                <a id="bootModalShow"
                                    ...
                                    class="btn btn-sm border border-slate-300 rounded-circle"
                            ...
                            <div class="col-lg-1 mt-2">
                                @if ($each?->send_status == true && ($each?->call == false || $each?->call_status == true))
                                    <i
                                        class="fs-4 text-success fa-regular fa-circle-check"></i>
                                @else
                                    <i class="fs-4 text-danger fa-solid fa-circle-xmark"></i>
                                @endif

                                <a id="detailModalShow"
                                    formActionUrl={{ $each->customer?->id }}
                                    href="{{ route('admin.user.detail', ['customer' => $each->customer?->id]) }}">
