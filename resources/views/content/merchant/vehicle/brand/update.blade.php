...
<div class="container">
    ...
    <div class="card rounded border border-gray-300">
        <div class="card-body">
            <form action="{{ route('merchant.vehicle.brand.update', ['brand' => $brand->id]) }}" method="POST" accept-charset="utf-8" autocomplete="on" class="need-validation" novalidate enctype="multipart/form-data">
                @csrf
                @method('put')
                <input type="hidden" name="image_id" value="{{ $brand?->image?->id }}" />
                <div class="form-group mb-3">
                    <label class="form-label fw-500 mb-1" for="title[en]">Title in English<span class="ms-1 text-red-600">*</span></label>
                    <input type="text" class="form-control" name="title[en]" value="{{ old('title.en', toLocaleString($brand->translate)) }}" required />
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" name="title[bn]" value="{{ old('title.bn', toLocaleString($brand->translate, 'bn')) }}" required />
                ...
                <div class="mt-4">
                    ...
                    <a href="{{ route('merchant.vehicle.brand.index') }}" class="btn btn-default">
