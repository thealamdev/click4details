...
<div class="container">
    <div class="d-flex align-items-center justify-content-between">
        <div>
            <h4 class="mb-0">Update Fuel</h4>
            <p>Prior to update resource, make sure asterisk (*) signs are filled</p>
        </div>
    </div>
    <div class="card rounded border border-gray-300">
        <div class="card-body">
            <form action="{{ route('merchant.vehicle.fuel.update', ['fuel' => $fuel->id]) }}" method="POST" accept-charset="utf-8" autocomplete="on" class="need-validation" novalidate>
                @csrf
                @method('put')
                <div class="form-group mb-3">
                    <label class="form-label fw-500 mb-1" for="title[en]">Title in English<span class="ms-1 text-red-600">*</span></label>
                    <input type="text" class="form-control" name="title[en]" value="{{ old('title.en', toLocaleString($fuel->translate)) }}" required />
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" name="title[bn]" value="{{ old('title.bn', toLocaleString($fuel->translate, 'bn')) }}" required />
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-select" name="status" required>
                        ...
                        <option value="1" @selected(old('status', $fuel->status)=='1' )>Active</option>
                        <option value="0" @selected(old('status', $fuel->status)=='0' )>Inactive</option>
                    </select>
                </div>
                <div class="mt-4">
                    ...
                    <a href="{{ route('merchant.vehicle.fuel.index') }}" class="btn btn-default">
