...
class CategoryHandler extends HandlerAdapter
{
    /**
    ...
    {
        $instance = Category::query()->create(array_merge_recursive($request->only('icon', 'link', 'is_feat', 'status'), [
    ...
    {
        $process = $instance instanceof Model ? $instance : Category::query()->findOrFail($instance->category);
    ...
    {
        $instance->update(array_merge_recursive($request->only('icon', 'link', 'is_feat', 'status'), [
            'slug' => $this->makeSlugString($request->input('title'))
        ]));
        $this->withStoreLocal($instance, $request->input('title'));
        return $instance;
