...
class ShippinglHandler extends HandlerAdapter
{
    /**
    ...
    {
        $instance = Shipping::query()->create(array_merge_recursive($request->only('charge','status'), [
    ...
    {
        $process = $instance instanceof Model ? $instance : Shipping::query()->findOrFail($instance->edition);
    ...
    {
        $instance->update(array_merge_recursive($request->only('charge','status'), [
            'slug' => $this->makeSlugString($request->input('title')).uniqid()
        ]));
        $this->withStoreLocal($instance, $request->input('title'));
        return $instance;
