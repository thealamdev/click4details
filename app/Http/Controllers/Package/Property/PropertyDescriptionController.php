...
class PropertyDescriptionController extends Controller
{
    ...
    {
        return view('content.package.property.product.detail', compact('property'));
    }
     ...
     */
    public function store(Property $property, StoreDescriptionRequest $request): RedirectResponse
    {
        if ($request->validated()) {
            $isManage = collect($request->input('description'))->each(fn ($each, $lang) => $property->description()->updateOrCreate(['local' => $lang], ['content' => $each, 'local' => $lang]));
            $response = $isManage ? 'Congrats! Data is processed successfully' : 'Oops! Unable to process record';
            return Redirect::route('admin.property.gallery.index', ['property' => $property->getKey()])->with('status', $response);
        }
